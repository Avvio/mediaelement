...
c=b.find("img");if(c.length==0)c=f('<img width="100%" height="100%" />').appendTo(b);c.attr("src",a)},buildoverlays:function(a,b,c,d){var e=this;if(a.isVideo){var g=f('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-loading"><span></span></div></div>').hide().appendTo(c),k=f('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-error"></div></div>').hide().appendTo(c),h=f('<div class="mejs-overlay mejs-layer mejs-overlay-play"><div class="mejs-overlay-button"></div></div>').appendTo(c).click(function(){if(e.options.clickToPlayPause){e.options.resetOnPlay&&
d.setCurrentTime(0);d.paused?d.play():d.pause()}});d.addEventListener("play",function(){h.hide();g.hide();b.find(".mejs-time-buffering").hide();k.hide()},false);d.addEventListener("playing",function(){h.hide();g.hide();b.find(".mejs-time-buffering").hide();k.hide()},false);d.addEventListener("seeking",function(){g.show();b.find(".mejs-time-buffering").show()},false);d.addEventListener("seeked",function(){g.hide();b.find(".mejs-time-buffering").hide()},false);d.addEventListener("pause",function(){mejs.MediaFeatures.isiPhone||
h.show()},false);d.addEventListener("waiting",function(){g.show();b.find(".mejs-time-buffering").show()},false);d.addEventListener("loadeddata",function(){g.show();b.find(".mejs-time-buffering").show()},false);d.addEventListener("canplay",function(){g.hide();b.find(".mejs-time-buffering").hide()},false);d.addEventListener("error",function(){g.hide();b.find(".mejs-time-buffering").hide();k.show();k.find("mejs-overlay-error").html("Error loading this resource")},false)}},buildkeyboard:function(a,b,
...
(function(f){f.extend(mejs.MepDefaults,{googleAnalyticsTitle:"",googleAnalyticsCategory:"Videos",googleAnalyticsEventPlay:"Play",googleAnalyticsEventPause:"Pause",googleAnalyticsEventEnded:"Ended",googleAnalyticsEventTime:"Time",googleAnalyticstrackingFunction:null});f.extend(MediaElementPlayer.prototype,{buildgoogleanalytics:function(a,b,c,d){d.addEventListener("play",function(){if(typeof _gaq!="undefined")a.options.googleAnalyticstrackingFunction(["_trackEvent",a.options.googleAnalyticsCategory,
a.options.googleAnalyticsEventPlay,a.options.googleAnalyticsTitle===""?a.currentSrc:a.options.googleAnalyticsTitle])},false);d.addEventListener("pause",function(){if(typeof _gaq!="undefined")a.options.googleAnalyticstrackingFunction(["_trackEvent",a.options.googleAnalyticsCategory,a.options.googleAnalyticsEventPause,a.options.googleAnalyticsTitle===""?a.currentSrc:a.options.googleAnalyticsTitle])},false);d.addEventListener("ended",function(){if(typeof _gaq!="undefined")a.options.googleAnalyticstrackingFunction(["_trackEvent",
a.options.googleAnalyticsCategory,a.options.googleAnalyticsEventEnded,a.options.googleAnalyticsTitle===""?a.currentSrc:a.options.googleAnalyticsTitle])},false)}})})(mejs.$);
